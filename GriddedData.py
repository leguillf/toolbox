...
class grid2D:
   ...
   def _get_gphiglam(self):
       """Get glam*,gphi* for * in t,u,v,f  
          Updated 18 April 2017, EC
       """
       self.glamt = self.navlon
       self.gphit = self.navlat
       gradlat_glamt, gradlon_glamt = npy.gradient(self.glamt)
       gradlat_gphit, gradlon_gphit = npy.gradient(self.gphit)
       self.glamu = self.glamt + gradlon_glamt/2.
       self.gphiu = self.gphit # Why this? + gradlon_gphit/2.
       self.glamv = self.glamt
       self.gphiv = self.gphit + gradlat_gphit/2.
       self.glamf = self.glamu
       self.gphif = self.gphiv
...
def corio(dom,grid='t'):
    ...
    """
    exec('global lat; lat = dom.gphi' + grid)
